...
class TestInMemoryLabReportRepository:
    ...
    @pytest.mark.asyncio
    async def test_get_lab_report(self, lab_report_repo: InMemoryLabReportRepository):
        result = await lab_report_repo.get_lab_report("any input")
    ...
    @pytest.mark.asyncio
    async def test_lab_report_has_abnormal_count(self, lab_report_repo: InMemoryLabReportRepository):
        result = await lab_report_repo.get_lab_report("test")
        assert "abnormal_count" in result
        assert isinstance(result["abnormal_count"], int)

    @pytest.mark.asyncio
    async def test_lab_report_summary(self, lab_report_repo: InMemoryLabReportRepository):
        result = await lab_report_repo.get_lab_report("test")
